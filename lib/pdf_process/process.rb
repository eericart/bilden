...
class PdfProcess::Process
  ...
  def process_data
    convert_to_csv
    (1..2).each do |index|
      csv_content=CSV.open("csv/#{index}.csv", "r").map{|content| content}
      csv_content.first.delete("")
      career_name = csv_content.delete_at(0).split if index == 1
  ...
  def clean_data (csv_data)
    ...
    csv_data.each {|a| a.insert(0,genete_clave(a[0])) if a.size == 3}
  ...
  def convert_to_record
    ...
    career=Career.find_by(code: "#{career_name[1]}") || Career.create(name: "#{career_name[0].capitalize}", code:"#{career_name[1]}" )
